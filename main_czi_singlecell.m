...
[file_name,file_dir,~] = uigetfile('.czi');

image_raw = ReadImage6D_timepoint(strcat([file_dir,file_name]),t_axis);
total_z = image_raw{1,2}.SizeZ;
display(image_raw{1,2}.SizeZ);
...
[sample_ref,rect_sample] = imcrop(imadjust(overlay_ref,image_adjust,[]));
 ...
 for n = 1:count-1 
    ...
    if hardstack ==0
        image_crop = imcrop(temp_image,rect_sample);
    else
...
%     [~,~,channel2_pixel_raw,~,~,total_intensity_ch2_raw] = overlap(channel1,channel2);
...
dlmwrite(strcat(file_name,'.csv'),[total_pixel,channel1_pixel,total_intensity_ch1,channel2_pixel,total_intensity_ch2,overlap_pixel],'-append','precision',5);
dlmwrite(strcat(file_name,'.csv'),[k1_per,k2_per,k1_per*k2_per],'-append','precision',5);
% dlmwrite(strcat(file_name,'.csv'),[totalz_pixels_ch2,totalz_intensity_ch2,t_axis],'-append','precision',5);
% dlmwrite(strcat(file_name,'_cluster','.csv'),[pixels_dist_mean,pixels_dist_var,t_axis],'-append','precision',5);
